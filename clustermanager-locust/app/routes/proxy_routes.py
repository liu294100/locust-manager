...
def proxy_to_instance(instance_id, path=''):
    ...
    if instance:
        ...
        try:
            ...
            if is_cluster_enabled():
                ...
                cluster_inst = cluster_manager.get_cluster_instance(instance_id)
                if cluster_inst and cluster_inst.port:
                    target_port = cluster_inst.port
                    # 判断是否在当前节点
                    current_node_id = node_registry.get_node_id()
                    if cluster_inst.node_id != current_node_id:
                        # 其他节点的实例，代理到对应 IP
                        node = node_registry.get_node(cluster_inst.node_id)
                        if node:
                            target_host = node.ip_address
                        else:
                            # 直接从 web_url 提取
                            if cluster_inst.web_url:
                                from urllib.parse import urlparse
                                parsed = urlparse(cluster_inst.web_url)
    ...
    try:
        ...
        timeout = (15, 300) if request.method in ['GET', 'HEAD', 'OPTIONS'] else (15, 300)
